...
def inpt(text, isURL):
    ...
    if isURL == 1:
        ...
        if not all([parsed_url.scheme, parsed_url.netloc]):
            loading_effect("invalid input!", 2)
            inpt("url: ", 1)
    else:
        pass
...
def invalid_character_number():
    print("Enter a valid category number!")
    print("It is not a valid category number.")
    loading_effect("restarting...", 2)
    main()


def menu_list(menu_name, menu_options):
    paas()
    print(f"== {menu_name} Menu ".ljust(56, "="))
    for key, value in menu_options.items():
        print(f"[{key}] {value}")
    print("========================================================")

    global cmd
    cmd = inpt("Select Lab: ", 0)

    global menu_header
    menu_header = f"\n== {menu_name}/{menu_options[cmd]} ==\n"

    return int(cmd), menu_header
...
def directory_traversal_labs():
    ...
    if 1 <= cmd <= 6:
        ...
        requests.get(image_url, verify=False, proxies=proxies)
        if "root:x" in s.text:
            loading_effect("attacking", 3)
...
def access_control_vulnerabilities_labs():
    ...
    if 1 <= cmd <= 13:
        ...
        if selected_lab == 2:
                ...
                loading_effect("deleting carlos's"
                               " account...", 1)
...
def main():
    ...
    try:
        inpt("Select Vulnerability: ", 0)

        if 1 <= cmd <= 4:
            try:
                eval('_'.join(menu_options[cmd].lower().split()) + "()")
            except Exception as e:
                # print error for development
                print(e)
                ex(0)
        else:
            loading_effect("invalid category number!", 2)
            main()
